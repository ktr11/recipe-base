...
| **3.5** | **認可ルールの統合テスト（§10.2）** |
| 4 | Next.js 側の認証基盤（Cookie ベースのトークン保存、`createServerRunner`） |
| 5 | Tailwind v4 + daisyUI 5、テーマ切り替えとちらつき対策（§6.2） |
| **5.5** | **テスト基盤（Vitest）の導入（§10.1）** |
| 6 | `RecipeRepository` インターフェースと `LocalStorageRepository`（トライアル制限を含む）+ 契約テスト |
| 7 | レシピ CRUD 画面 + 検索 + x人前スケーリング（ゲストで完結する状態まで）+ ロジックのテスト |
...
テストの差し込み位置（3.5 と 5.5）を選んだ理由は §10 に記す。

---

## 10. テスト方針

テストは2種類に分け、**目的も実行環境も分離する**。混ぜると、AWS への接続が必要なテストのせいで単体テストが遅くなり、誰も実行しなくなる。

| 種別 | コマンド | 対象 | 実行環境 |
|---|---|---|---|
| 単体テスト | `pnpm test` | 純粋なロジック | ローカル完結。AWS 不要 |
| 統合テスト | `pnpm test:integration` | 認可ルール | デプロイ済みの sandbox に対して実行 |

**ツールは Vitest を使用する。** Next.js 16 / ESM 環境での設定が Jest より大幅に少なく、起動が速い。React コンポーネントのテストが必要になった時点で Testing Library を追加する。

### 10.1 単体テスト（ステップ 5.5 で基盤導入）

**ステップ6の直前に基盤を入れる。** 理由は、ステップ6が `RecipeRepository` インターフェースを定義する場所だからである。

`LocalStorageRepository`（ステップ6）と `AmplifyRepository`（ステップ8）は**同じインターフェースを実装し、同じ振る舞いをしなければならない**。ステップ6の時点でテストを書けば、それは単体テストではなく**両実装が満たすべき契約テスト**になり、ステップ8にそのまま適用できる。

```
describe.each([
  ['LocalStorageRepository', () => new LocalStorageRepository()],
  ['AmplifyRepository',      () => new AmplifyRepository()],
])('%s', (_name, create) => { /* 共通の契約 */ })
```

ステップ8を書き終えてから追加すると、この構造は自然には現れない。「動いているコードに後からテストを書く」作業になり、実装がテストしやすい形に寄る力が働かない。

**主なテスト対象**（いずれも AWS もブラウザも不要な純粋関数に切り出す）:

| 対象 | ステップ | 要点 |
|---|---|---|
| トライアル制限の判定 | 6 | 3/3/10 の境界。削除後に再作成できること（累計ではなく同時保有数） |
| 検索フィルタ・正規化 | 7 | 全角/半角・ひらがな/カタカナの吸収。複数条件の AND |
| x人前スケーリング | 7 | **`quantity: null` をスケールしない**分岐、小数の丸め |
| ラベル ID の付け替え | 9 | 移行時にラベルが外れないこと。`migrated` フラグによる冪等性 |
| 招待コードの生成 | 10 | 除外文字（`0` `O` `1` `I` `L`）が出現しないこと |

この一覧は、本設計書が「壊れやすい」と名指しした箇所とほぼ一致している。

### 10.2 認可ルールの統合テスト（ステップ 3.5）

**本プロジェクトで最も価値が高く、かつ単体テストでは原理的に検証できないテスト。**

`groupDefinedIn` による認可（§1.2）が誤っていた場合の帰結は「他人の家族のレシピが見える」であり、影響が最も重い。そしてこれは**モックでは何も証明できない** —— 実際にデプロイされた AppSync に対してリクエストを投げるしかない。

**ステップ3の直後に置く理由**: `postConfirmationFunction` が完成して初めて「チームに所属した実ユーザー」を作れるようになる。それ以前は検証対象が存在しない。

**検証するシナリオ**（主眼は「**できないこと**」の確認にある）:

- チーム A のユーザーが、チーム B のレシピを `list` で取得**できない**
- チーム A のユーザーが、チーム B のレシピを ID 直指定で `get` **できない**
  （一覧に出ないことと、取得できないことは別の保証である）
- チーム A のユーザーが、チーム B のレシピを更新・削除**できない**
- 同一チームの別ユーザーが、互いのレシピを編集**できる**
- チームに所属しないユーザーが、`Team` レコードを読んで招待コードを列挙**できない**
- 有効期限切れの招待コードで `joinTeam` が**失敗する**（§2.3）

### 10.3 テストを書かないもの

意図的に対象外とする。聞かれない限り追加しない。

- **UI コンポーネントのスナップショットテスト** — 見た目を変えるたびに更新するだけの作業になり、バグを検出しない
- **Amplify の generated client 自体の動作** — AWS の責任範囲であり、こちらで検証する意味がない
- **E2E（Playwright）** — v1 では見送る。「ゲストで作成 → 登録 → 引き継ぎ」の1本のみ価値があるが、CI で Cognito のメール確認コードを受け取る仕組みが必要になり、投資に見合わない
